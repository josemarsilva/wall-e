package org.gnu.automation.walle.dataSources;
...
public class WorkbookSheetTable {
	...
	private TableOf tableOfWorkbookSheetTable = new TableOf();
	...
	public void load() throws Exception {
		...
		if (getWorkbookFileName()==null) {
			throw new Exception(org.gnu.automation.walle.scriptLanguage.structures.StructuresConstants.MSG_TXT_ERROR_WORKBOOKSHEETTABLE_FILENAME_CANT_BE_NULL);
		} else {
			try {
				this.workbook = WorkbookFactory.create(new FileInputStream(getWorkbookFileName().replace("\\", "//")));
			} catch (Exception e) {
				e.printStackTrace();
				throw new Exception(org.gnu.automation.walle.scriptLanguage.structures.StructuresConstants.MSG_TXT_ERROR_WORKBOOKSHEETTABLE_LOAD_EXCEPTION.replaceFirst("%s", e.getClass().getName()).replaceFirst("%s", getWorkbookFileName()));
			}
		}
	...
	public TableOf getTableOfWorkbookSheetTable(String wsName) throws Exception {
		...
		if (sheet==null) {
			throw new Exception(org.gnu.automation.walle.scriptLanguage.structures.StructuresConstants.MSG_TXT_ERROR_WORKBOOKSHEETTABLE_SHEET_DOES_NOT_EXISTS.replaceFirst("%s", (wsName==null) ? "" : wsName).replaceFirst("%s", this.getWorkbookFileName()));
		}
